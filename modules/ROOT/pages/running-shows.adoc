...
There is another soft limit in the open-source version of the server written in Python, which can become painfully slow and unreliable above around 500-800 drones, depending on your hardware.
For such large shows we strongly encourage you to buy a professional license, which includes several components rewritten in Rust for high reliability and enhanced performance, among many other useful features required by professional drone show operations and several flight authorities.
